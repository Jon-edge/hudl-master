...
export function WinCelebration({
  ...
  useEffect(() => {
    ...
    // Small delay to let DOM elements render and get their actual positions
    const initTimeout = setTimeout(() => {
      ...
      // Measure actual avatar position from DOM
      if (avatarRef.current) {
        ...
        const avatarBody = Bodies.circle(avatarCenterX, avatarCenterY, avatarRadius, {
          isStatic: true,
          restitution: 0.8,
          label: "avatar",
        })
      ...
      // Measure actual text block position from DOM
      if (textBlockRef.current) {
        ...
        const nameBody = Bodies.rectangle(textCenterX, textCenterY, textRect.width, textRect.height, {
          isStatic: true,
          restitution: 0.6,
          chamfer: { radius: 10 },
          label: "name",
  ...
  return (
    ...
    >
      ...
      <div 
        className="relative z-10 flex flex-col items-center gap-6 p-8 pointer-events-none"
      >
        {/* Avatar - ref on the actual image for physics measurement */}
        <div className="relative animate-bounce-subtle">
          ...
          <div ref={avatarRef} className="relative w-40 h-40">
            <Image
              ...
              height={160}
              unoptimized
              className="w-full h-full rounded-full object-cover ring-4 ring-amber-400 shadow-2xl"
            />
          </div>
        ...
        {/* Winner text - ref on block for physics measurement */}
